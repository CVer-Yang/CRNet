...
def CE_Net_Train():
    # NAME = 'CE-Net' + Constants3.ROOT.split('/')[-1]
    NAME = 'CRNet' + Constants3.ROOT.split('/')[-1]
    print(NAME)
    # run the Visdom
    ...
    batchsize = torch.cuda.device_count() * Constants3.BATCHSIZE_PER_CARD
    ...
    #############修改自己的数据集地址
    dataset = ImageFolder(root_path=Constants3.ROOT, datasets='Space')
    ...
    no_optim = 0
    total_epoch = Constants3.TOTAL_EPOCH
    train_epoch_best_loss = Constants3.INITAL_EPOCH_LOSS
    for epoch in range(1, total_epoch + 1):
        ...
        print('SHAPE:', Constants3.Image_size, file=mylog)
        print('********')
        print('epoch:', epoch, '    time:', int(time() - tic))
        print('train_loss:', train_epoch_loss)
        print('SHAPE:', Constants3.Image_size)
        ...
        if no_optim > Constants3.NUM_EARLY_STOP:
            print('early stop at %d epoch' % epoch, file=mylog)
            print('early stop at %d epoch' % epoch)
            break
        if no_optim > Constants3.NUM_UPDATE_LR:
