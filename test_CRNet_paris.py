...
def test_ce_net_vessel():
    source = './dataset/paris_build/test/images/'
    val = os.listdir(source)
    disc = 20
    solver = TTAFrame(CRNet)
    #加载训练模型权重
    solver.load('./weights/CRNetparis_build.th')
    ...
    target = './submits/CRNetNet_paris/'
    if not os.path.exists(target):
        os.mkdir(target)
    #gt_root = '/data/guoxuejun/yangjialin/CE-Net/dataset/ROAD/test/masks'
    gt_root = './dataset/paris_build/test/road_mask'
    ...
    for i, name in enumerate(val):
        ...
        ground_truth_path = os.path.join(gt_root, name.replace('_image.png','_labels.jpg'))
