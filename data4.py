...
def read_Parisroad_datasets(root_path, mode='train'):
    images = []
    masks = []
    ...
    for image_name in os.listdir(image_root):
        if not image_name.startswith("build"):  # 检查文件名是否以 "build" 开头
            image_path = os.path.join(image_root, image_name.split('.')[0] + '.png')
            label_path = os.path.join(gt_root, image_name.split('.')[0].replace('image','labels')+'.jpg')
...
def read_MasBuild_datasets(root_path, mode='train'):
    images = []
    masks = []

    image_root = os.path.join(root_path, 'training/images/')
    gt_root = os.path.join(root_path, 'training/masks1/')
    for image_name in os.listdir(image_root):
        image_path = os.path.join(image_root, image_name.split('.')[0] + '.tiff')
        label_path = os.path.join(gt_root, image_name.split('.')[0] + '.tif')
...
def read_miniDeep_datasets(root_path, mode='train'):
    images = []
    masks = []
...
class ImageFolder(data.Dataset):
    ...
    def __init__(self, root_path, datasets='Messidor',  mode='train'):
        ...
        assert self.dataset in ['RIM-ONE', 'Messidor', 'ORIGA', 'DRIVE', 'Cell', 'Vessel','Mas', 'CHG','miniDeep','MasBuild','paris_road'], \
        ...
        elif self.dataset == 'miniDeep':
            self.images, self.labels = read_miniDeep_datasets(self.root, self.mode)
        elif self.dataset == 'MasBuild':
            self.images, self.labels = read_MasBuild_datasets(self.root, self.mode)
        elif self.dataset == 'paris_road':
            self.images, self.labels = read_Parisroad_datasets(self.root, self.mode)
        else:
